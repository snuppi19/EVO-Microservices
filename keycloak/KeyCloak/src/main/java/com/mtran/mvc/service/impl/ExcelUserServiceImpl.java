...
import com.mtran.mvc.service.ExcelUserService;
import com.mtran.mvc.validator.UserImportValidator;
...
public class ExcelUserServiceImpl implements ExcelUserService {
    ...
    public byte[] exportUsersToExcel(String username, String fullName, LocalDate createdDateFrom, LocalDate createdDateTo, Integer experienceYears) throws Exception {
        List<CustomUser> users = userRepository.findAll(buildSpecification(username, fullName, createdDateFrom, createdDateTo, experienceYears));
    ...
    public int importUsersFromExcel(MultipartFile file) {
        List<CustomUser> validUsers = new ArrayList<>();
        ...
        try (Workbook workbook = new XSSFWorkbook(file.getInputStream())) {
            ...
            for (int i = 1; i <= sheet.getLastRowNum(); i++) {
                ...
                CustomUser user = new CustomUser(dto.getUsername(), dto.getFullName(), birthDate,
    ...
    private Specification<CustomUser> buildSpecification(String username, String fullName, LocalDate createdDateFrom, LocalDate createdDateTo, Integer experienceYears) {
